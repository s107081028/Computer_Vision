...
def estimate_initial_RT(E):
    ...
    U, D, Vt = np.linalg.svd(E)                       # Apply SVD
    W = np.array([[0,-1,0], [1,0,0], [0,0,1]])        # Define W
    Q1 = U.dot(W).dot(Vt)                             # Q1 = UWVt
    R1 = np.linalg.det(Q1)*Q1                         # R1 = det(Q1)*Q1
    Q2 = U.dot(W.T).dot(Vt)                           # Q2 = UWtVt
    R2 = np.linalg.det(Q2)*Q2                         # R2 = det(Q2)*Q2
    
    T1 = U[:, 2].reshape(3, 1)           # T = u3
    T2 = -T1                             # T = -u3
    RT = np.zeros((4, 3, 4))             # 4 combinations
...
def linear_estimate_3d_point(image_points, camera_matrices):
    # TODO: Implement this method!
    M = image_points.shape[0]                       # # of images
    DLTmatrix = []

    for i in range(M):
        # vMi3 - Mi2 
        DLTmatrix.append(image_points[i][1] * camera_matrices[i][2] - camera_matrices[i][1])
        # Mi1 - uMi3
        DLTmatrix.append(camera_matrices[i][0] - image_points[i][0] * camera_matrices[i][2])
    
    DLTmatrix = np.array(DLTmatrix)
    _, __, Vt = np.linalg.svd(DLTmatrix)            # SVD solving
    return (Vt[-1] / Vt[-1, -1])[:3]                # Normalize Vt and return 3d point
...
def reprojection_error(point_3d, image_points, camera_matrices):
    # TODO: Implement this method!
    P = np.hstack((point_3d, 1))                      # P = [X,Y,Z,1]
    error = []
    for i in range(image_points.shape[0]):
        Y = camera_matrices[i].dot(P)                 # y = MiP
        Pi_ = np.array([Y[0], Y[1]]) / Y[2]           # (1 / y3) * [y1, y2]
        error.append((Pi_ - image_points[i])[0])      # pi' - pi
        error.append((Pi_ - image_points[i])[1])
...
def jacobian(point_3d, camera_matrices):
    ...
    Jacob = []
    for i in range(camera_matrices.shape[0]):                           # Jacobian Matrix
        Mi = camera_matrices[i]
        Pi = Mi.dot(P)
        Jacob.append(((Pi[2] * Mi[0] - Pi[0] * Mi[2]) / Pi[2]**2)[:3])
        Jacob.append(((Pi[2] * Mi[1] - Pi[1] * Mi[2]) / Pi[2]**2)[:3])
...
def nonlinear_estimate_3d_point(image_points, camera_matrices):
    # TODO: Implement this method!
    P_estiamte = linear_estimate_3d_point(image_points, camera_matrices)        # Estimate Point
    for i in range(10):                                                         # 10 iterations
        error = reprojection_error(P_estiamte.copy(), image_points, camera_matrices)   # Calculate Error
        J = jacobian(P_estiamte.copy(), camera_matrices)                               # Calculate Jacobian Matrix
        P_estiamte -= np.linalg.inv(J.T.dot(J)).dot(J.T).dot(error)             # P = P - (JtJ)^(-1) * Jt * e
...
def estimate_RT_from_E(E, image_points, K):
    ...
    positive = np.zeros((RT.shape[0]))
    camera_matrices = np.zeros((2, 3, 4))
    camera_matrices[0, :, :] = K.dot(np.hstack((np.eye(3), np.zeros((3,1)))))             # Camera Matrix 1

    for i in range(RT.shape[0]):
        camera_matrices[1, :, :] = K.dot(RT[i])                                           # Camera Matrix 2
        for j in range(image_points.shape[0]):
            points_3d = nonlinear_estimate_3d_point(image_points[j], camera_matrices)
            # points_3d = linear_estimate_3d_point(image_points[j], camera_matrices)
            points_3d_2 = RT[i].dot(np.append(points_3d, 1.).T)                           # Camera 2 Coordinate
            if points_3d[2] > 0 and points_3d_2[2] > 0:
                positive[i] += 1

    return RT[np.argmax(positive)]
